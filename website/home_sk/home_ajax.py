...
from . import home_sk
from .home_models import SHSKSensorData
from website import app

class SDEncoder(json.JSONEncoder):
    def default(self, obj):
        if isinstance(obj, SHSKSensorData):
...
def get_sensor_data():
    ...
    result = SHSKSensorData.query\
        .filter(or_(SHSKSensorData.sensor_name==sensor_name, sensor_name=='all'))\
        .filter(and_(SHSKSensorData.time_stamp>start_time, SHSKSensorData.time_stamp<end_time))\
        .order_by(SHSKSensorData.time_stamp).limit(limit).all()
